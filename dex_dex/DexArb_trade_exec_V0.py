...
class ws_dex():
    def __init__(self):
        self.web3 = Web3(Web3.WebsocketProvider("wss://wsapi.fantom.network/"))#wss://rough-sparkling-cloud.fantom.quiknode.pro/5514dc804258c85e2ffdf69cdf594f6f3c0596a9/ - wss://wsapi.fantom.network/
        ...
        self.arb_cntrct = self.web3.eth.contract(address='0x2cdeE62cB6A5a4dd2e8852B8460E728B50421913', abi=load(open("./abi/arb_contract_not_pay.json")))
        self.tx_signed_sushi_spooky = self.create_tx([self.router_adr['SushiSwapRouter'], self.router_adr['SpookySwapRouter']])
        self.tx_signed_spooky_sushi = self.create_tx([self.router_adr['SpookySwapRouter'], self.router_adr['SushiSwapRouter']])
    ...
    def get_TradeInfo(self):
        ...
        end_time = time.time()
        print('get_trade_info duration: ' + str(end_time - start_time))
    ...
    def handle_event(self, event):
        ...
        trade_info = self.get_TradeInfo()
        print(trade_info)
        if trade_info['SpiritSwap->SushiSwap'] > 0.75:
            self.webhook.send('block_number pre-tx: ' + str(self.web3.eth.get_block_number()))
            receipit = self.do_trade(self.create_tx([self.router_adr['SushiSwapRouter'], self.router_adr['SpookySwapRouter']]))
            self.webhook.send('info trade - SpiritSwap->SushiSwap : ' + str(trade_info))
            self.webhook.send('block_number post-tx: ' + str(self.web3.eth.get_block_number()))
            self.webhook.send('tx_hash:' + str(self.web3.toHex(receipit)))
            end_time = time.time()
            self.webhook.send('get_handle_event Duration: ' + str(end_time - start_time))
        elif trade_info['SpiritSwap->SushiSwap'] > 0.75:
            self.webhook.send('block_number pre-tx: ' + str(self.web3.eth.get_block_number()))
            receipit = self.do_trade(self.create_tx([self.router_adr['SpookySwapRouter'], self.router_adr['SushiSwapRouter']]))
            self.webhook.send('info trade - SpiritSwap->SushiSwap : ' + str(trade_info))
            self.webhook.send('block_number post-tx: ' + str(self.web3.eth.get_block_number()))
            self.webhook.send('tx_hash:' + str(self.web3.toHex(receipit)))
            end_time = time.time()
    ...
    def create_tx(self, dex_path):
        ...
        signed_tx = self.web3.eth.account.sign_transaction(tx, private_key=pvt_key)
        #print(self.web3.toHex(signed_tx['hash']))
        return signed_tx

#todo: controllare se 'await_tx_recepit()' può creare delay
    def do_trade(self, tx_signed):
        start_time = time.time()
        tx_hash = self.web3.eth.sendRawTransaction(tx_signed.rawTransaction)
        end_time = time.time()
    ...
    def get_mempool(self):
        ...
        for adr in pending:
            for tx in list(pending[adr].keys()):
                ...
                print(pending_tx['gas'])
                gas = self.web3.toInt(hexstr = pending_tx['gas'])
                gas_price = self.web3.toInt(hexstr = pending_tx['gasPrice'])
                if best_gas and best_gasPrice == 0:
                    best_gas = gas
                    best_gasPrice = gas_price
                elif best_gas < gas and best_gasPrice < gas_price:
                    best_gas = gas
                    best_gasPrice = gas_price
        end_time = time.time()
        print('time_exec: ' + str(end_time - start_time))
        return{'gas': best_gas, 'gasPrice': gas_price, 'FTM to pay' : self.web3.fromWei(best_gas * gas_price, 'ether')}
    ...
    def main(self):
        ...
        #self.create_tx([self.router_adr['SushiSwapRouter'], self.router_adr['SpookySwapRouter']])
